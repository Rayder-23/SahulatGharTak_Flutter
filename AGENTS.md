...
- **Client Addresses (with GPS pin-drop)** — `ClientAddressProvider` → `ClientAddressRepository` (`lib/data/repositories/client_address_repository.dart`) → `ClientAddressApiService`, `lib/models/client_address.dart` (`latitude`/`longitude` are nullable, plus a read-only `hasLocation` bool). `lib/screens/add_address_screen.dart` shows a `GoogleMap` (tap/drag to place a pin, defaults to device GPS via `geolocator`/`lib/services/location_permission_service.dart` else Karachi, built-in "my location" button once permission is granted) via the shared `lib/widgets/address_pin_map_view.dart`, plus a full-screen expand button (`lib/screens/pin_location_fullscreen_screen.dart`) for a more precise pin. An explicit "Save Pin" button reverse-geocodes via `lib/services/geocoding_api_service.dart` (`GET /api/geocoding/reverse`) to pre-fill Area/Full Address/City — still user-editable — before "Save Address" submits everything together. `lib/screens/profile_screen.dart`'s Addresses list shows a pin icon when `hasLocation` is true, a "Set location" chip when not.
...
- `permission_handler` — camera and location runtime permissions (`CameraPermissionService`, `LocationPermissionService`)
- `google_maps_flutter` — map rendering for the client address GPS pin-drop (`add_address_screen.dart`); requires a Google Cloud Maps API key per platform, wired into `android/app/src/main/AndroidManifest.xml` and `ios/Runner/AppDelegate.swift`
- `geolocator` — device GPS position to center the pin-drop map
